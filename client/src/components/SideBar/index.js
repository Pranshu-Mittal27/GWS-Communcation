		...
		<aside id="layout-menu" className="layout-menu menu-vertical menu bg-menu-theme">
			<div className="app-brand demo">
				<Link className="app-brand-link" to={"/"}>
					...
					<span className="app-brand-text demo menu-text fw-bolder ms-2">Searce</span>
				</Link>
			...
			<ul className="menu-inner py-1">
				{/* <!-- Dashboard --> */}
				{
					((navlinks !== undefined) && Array.isArray(navlinks)) &&
					navlinks.map((navlink, index) => {
						return (
							<li key={index} className={"menu-item" + ((location.pathname === navlink.path) ? " active" : "")}>
								<Link to={navlink.path} className="menu-link">
									<i className="menu-icon tf-icons bx bx-home-circle"></i>
									<div data-i18n="Analytics">{navlink.name}</div>
								</Link>
							</li>
						);
					})
...

				{
					...
					<li className="menu-item menu-item">
						<ul className="menu-link" onClick={(e) => handleLogout(e)}>
							<i className="menu-icon tf-icons bx bx-home-circle"></i>
